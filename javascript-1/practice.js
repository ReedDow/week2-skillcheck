...
let total = myNumbers.reduce((tot, num) => tot + num, 0)
...
let forTheLoveOfGeorge = notGeorge.map(nameItem =>  {
	return nameItem = 'George'
} )
...
let enemies = people.filter(objItem => {
	if(objItem.friend === false){
		return objItem
	}
} )
...
let totallyAwesome = people.reduce((tot, num) => tot + num.awesomeLevel, 0)
